...
- [ ] The loop produces the failure mode described — not a different failure nearby. Wrong bug = wrong fix.
...
Show the ranked list before testing. Proceed with your ranking if no response available.
...
A correct seam exercises the **real bug pattern** as it occurs at the call site. If no correct seam exists, note it — the architecture is preventing the bug from being locked down.
