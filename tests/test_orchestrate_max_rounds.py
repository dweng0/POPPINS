...
def test_cli_max_rounds_overrides_config():
    ...
    max_rounds = (
        cli_max_rounds
        if cli_max_rounds
        else config.get("orchestration", {}).get("max_rounds", 1)
    )
...
def test_cli_max_rounds_with_popins_config():
    ...
    effective = (
        cli_max_rounds
        if cli_max_rounds
        else poppins_config.get("orchestration", {}).get("max_rounds", 1)
    )
...
def test_cli_max_rounds_preserves_other_config():
    ...
    effective_max_agents = poppins_config.get("orchestration", {}).get(
