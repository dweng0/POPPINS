...
def test_sort_issues_by_reaction_count():
    ...
    assert result.index("### Issue #2: High reaction") < result.index(
        "### Issue #3: Medium reaction"
    )
    assert result.index("### Issue #3: Medium reaction") < result.index(
        "### Issue #1: Low reaction"
    )
