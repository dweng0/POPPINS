...
def test_release_workflow_exists():
    ...
    if os.path.exists(workflows_dir):
        ...
        assert os.path.exists(release_yml), (
            f"release.yml should exist in {workflows_dir}"
        )
...
def test_workflow_uses_setup_rust():
    ...
    if os.path.exists(workflows_dir):
        for yml_file in glob.glob(os.path.join(workflows_dir, "*.yml")):
            ...
            if "rust" in content.lower() and (
                "toolchain" in content.lower() or "rustup" in content.lower()
            ):
                assert True
                return
