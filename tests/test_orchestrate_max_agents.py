...
def test_cli_max_agents_overrides_config():
    ...
    max_agents = (
        cli_max_agents
        if cli_max_agents
        else config.get("orchestration", {}).get("max_parallel_agents", 1)
    )
...
def test_cli_max_agents_with_popins_config():
    ...
    effective = (
        cli_max_agents
        if cli_max_agents
        else poppins_config.get("orchestration", {}).get("max_parallel_agents", 1)
    )
...
def test_orchestrator_round_with_max_agents():
    ...
    for round_num in range(1, max_rounds + 1):
        ...
        if not selected:
            break
        assert len(selected) <= max_agents, (
            f"Round {round_num} selected {len(selected)} > {max_agents}"
        )
