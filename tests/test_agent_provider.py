...
"""
    ...
    try:
        ...
        assert result.stdout.strip() == "custom", (
            f"Expected 'custom', got '{result.stdout.strip()}'"
        )
    finally:
...
""".replace("SCRIPTS_DIR", scripts_dir)
        ...
        try:
            ...
            assert result.returncode == 0, f"Script failed: {result.stderr}"
            assert result.stdout.strip() == "provider:custom", (
                f"Expected 'provider:custom', got '{result.stdout.strip()}'"
            )
        finally:
            os.unlink(script_path)
        ...
        assert "ERROR: CUSTOM_MODEL not set" in result.stderr, (
            f"Expected CUSTOM_MODEL error, got: {result.stderr}"
        )
...
""".replace("SCRIPTS_DIR", scripts_dir)
    ...
    try:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            assert result.returncode == 0, f"Script failed: {result.stderr}"
            assert result.stdout.strip() == "provider:None", (
                f"Expected 'provider:None', got '{result.stdout.strip()}'"
            )
            ...
            assert "ERROR: No API key found" in result.stderr, (
                f"Expected error message, got: {result.stderr}"
            )
            assert "ANTHROPIC_API_KEY" in result.stderr, (
                "Error should list ANTHROPIC_API_KEY"
            )
            assert "MOONSHOT_API_KEY" in result.stderr, (
                "Error should list MOONSHOT_API_KEY"
            )
            assert "DASHSCOPE_API_KEY" in result.stderr, (
                "Error should list DASHSCOPE_API_KEY"
            )
...
""")
            ...
            assert "OLLAMA_HOST:http://localhost:11434" in output, (
                f"Expected 'OLLAMA_HOST:http://localhost:11434', got '{output}'"
            )
    finally:
        os.unlink(script_path)
...
"""
    ...
    try:
        ...
        assert "provider:dashscope" in output, (
            f"Expected 'provider:dashscope', got '{output}'"
        )
        assert (
            "base_url:https://dashscope-intl.aliyuncs.com/compatible-mode/v1" in output
        ), f"Expected correct base_url, got '{output}'"
    finally:
        os.unlink(script_path)
...
def test_missing_anthropic_package_error():
    ...
    assert "pip install anthropic" in content, "Installation instruction should exist"
...
""".replace("SCRIPTS_DIR", scripts_dir)
        ...
        try:
            with tempfile.TemporaryDirectory() as tmpdir:
                ...
                shutil.copy(
                    os.path.join(scripts_dir, "parse_poppins_config.py"),
