    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        try:
            ...
            assert os.environ.get("KEY") == "value", (
                f"Expected KEY=value, got KEY={os.environ.get('KEY')}"
            )

            # Verify ANOTHER_KEY is set
            assert os.environ.get("ANOTHER_KEY") == "another_value", (
                f"Expected ANOTHER_KEY=another_value, got ANOTHER_KEY={os.environ.get('ANOTHER_KEY')}"
            )

            # Verify comment lines were not parsed as keys
            assert "This" not in os.environ, (
                "Comment line should not be parsed as env var"
            )
